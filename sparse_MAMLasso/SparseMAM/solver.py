...
import sys
import pickle
from .utils import  division_tasks, build_M_dist, projection_simplex


########## MAM algorithm ############
# Optimized Operator Splitting method
def MAM(b, M_dist=False, exact=False, rho=1000, lasso=0, gamma=0, keep_track=True, evry_it = 10, name='MAM.pkl', visualize=False, computation_time=100,
    ...
    *iterations_k: (int) number of iterations 
    ...
    while (iterations_k < iterations_min) or (spent_time < computation_time and iterations_k < iterations_max and evol_p>precision): # and count_stop<10):  #  # and evol_p>10**-6) : # and evol_p>10**-16
        ...
        # To add a regularization and get a sparse barycenter
        if lasso > 0:
            I1 = sum_theta_mean > lasso
            I2 = sum_theta_mean < -lasso
            p[I1] = (sum_theta_mean[I1] - lasso) / inv_sum_S
            p[I2] = (sum_theta_mean[I2] + lasso) / inv_sum_S
